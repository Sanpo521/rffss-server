...
CREATE TABLE `nf_rffssp`  (
                             ...
                             `uniscid` varchar(50) NOT NULL COMMENT '统一社会信用代码',
                             `entname` varchar(200) NOT NULL COMMENT '冷藏冷冻库名称',
                             `address` varchar(300) NOT NULL COMMENT '冷藏冷冻库地址',
                             `storcapa` varchar(400) NULL COMMENT '贮存能力',
                             `legalrep` varchar(100) NULL COMMENT '法定代表人或者负责人姓名',
                             `callinfo` varchar(20) NULL COMMENT '联系方式',
                             `status` tinyint(2) NOT NULL COMMENT '状态 1 录入  2 提交  3 业务完成',
                             `createtime` datetime(0) NULL DEFAULT NULL COMMENT '创建时间',
                             `lasttime` datetime(0) NULL DEFAULT NULL COMMENT '最后操作时间',
                             `valid` int(1) NOT NULL COMMENT '是否有效：1有效 0无效',
                             `issueorg` varchar(10) NULL COMMENT '管理机构ID',
                             PRIMARY KEY (`id`) USING BTREE,
                             INDEX `uniscid`(`uniscid`) USING BTREE,
                             INDEX `legalrep`(`legalrep`) USING BTREE,
                             INDEX `callinfo`(`callinfo`) USING BTREE
